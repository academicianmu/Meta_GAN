...
class MonteCarloEvaluator:
    def __init__(self, real_data, num_samples=1000, batch_size=64):
        """
        初始化评估器
...
# 在训练循环中集成经验池和蒙特卡洛评估
def train_gan(dataset, num_epochs=800, batch_size=64, latent_dim=128, device=DEVICE):
    """训练主函数"""
    # 初始化模型
    ...
    # 经验池和蒙特卡洛评估器（传入真实数据）
    real_data_np = dataset.clean_data.numpy()  # 获取真实数据（numpy格式）
    experience_pool = ExperiencePool(max_size=1000)
    mc_evaluator = MonteCarloEvaluator(real_data=real_data_np, num_samples=100, batch_size=16)  # 调整评估样本数
...
def plot_results(dataset, generator, scalers, device=DEVICE):
    ...
    # 用电量分布对比
    plt.subplot(2, 2, 2)
    plt.hist(dataset.clean_data[:, 1].numpy(), bins=50, alpha=0.6, label='真实数据', color=colors[0])
    plt.hist(generated_data[:, 1], bins=50, alpha=0.6, label='生成数据', color=colors[1])
    plt.title('用电量分布对比', fontsize=12, fontweight='bold')
    plt.xlabel('用电量（MWh）', fontsize=10)
    plt.ylabel('频数', fontsize=10)
    plt.xlim(left=0)
    plt.legend()
    ...
    # 保存生成的特征数据
    np.savetxt('mc_generated_data.csv', generated_data, delimiter=',', header='总排放量,用电量(MWh),用烟煤量（吨）,用柴油量（吨）', comments='')
    # 计算均值和标准差并对比
    mean_fake = generated_data.mean(axis=0)
    std_fake = generated_data.std(axis=0)
    mean_real = dataset.clean_data.mean(axis=0)
    std_real = dataset.clean_data.std(axis=0)

    # 追加统计信息到CSV文件
    with open('mc_generated_data.csv', 'a') as f:
        f.write('\n# 均值和标准差对比\n')
