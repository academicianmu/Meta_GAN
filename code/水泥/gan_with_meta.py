...
class CementDataset(Dataset):
    def __init__(self, file_path):
        ...
        # 保存清洗后的原始数据用于对比
        self.clean_original_data = self._get_clean_original_data()
    ...
    def _get_clean_original_data(self):
        """获取清洗后的原始数据用于分布对比"""
        df_clean = self.df[self.features].fillna(self.df[self.features].mean()).clip(lower=0)
...
class Generator(nn.Module):
    def __init__(self, latent_dim=100):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(512, 4),
            nn.Sigmoid()  # 保持输出在[0,1]范围，与归一化范围匹配
        )
...
def plot_results(dataset, generator, g_losses, d_losses):
    ...
    # 确保生成数据非负
    fake_data_original = np.clip(fake_data_original, a_min=0, a_max=None)
    # 使用预处理时保存的原始清洗数据
    real_data_original = dataset.clean_original_data
    ...
    for i in range(1, 5):
        plt.subplot(2, 2, i)
        plt.hist(real_data_original[:, i-1], bins=30, alpha=0.5, label='真实数据')
        plt.hist(fake_data_original[:, i-1], bins=30, alpha=0.5, label='生成数据')
        plt.xlabel('数值')
        plt.ylabel('频数')
        plt.title(f'{features[i-1]}分布对比')
        plt.legend()
        plt.xlim(left=0)  # 确保x轴从0开始，符合业务逻辑
    ...
    # 保存生成的特征数据
    np.savetxt('cement_data.csv', fake_data_original, delimiter=',', header='总排放量,用电量(MWh),用烟煤量（吨）,用柴油量（吨）', comments='')
    # 计算均值和标准差并对比
    ...
    # 追加统计信息到CSV文件
    with open('cement_data.csv', 'a') as f:
        f.write('\n# 均值和标准差对比\n')
