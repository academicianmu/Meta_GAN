...
class CementDataset(Dataset):
    def __init__(self, file_path):
        self.df = pd.read_excel(file_path, sheet_name="钢铁(原始数据）")
        self.features = ["总排放量(t)", "用电量(MWh)", "废钢消耗量(t)", "一般烟煤消耗量(t)"]
...
def plot_results(dataset, generator, g_losses, d_losses):
    ...
    plt.subplot(2, 2, 1)
    plt.hist(real_data_original[:, 0], bins=30, alpha=0.5, label='真实数据')
    plt.hist(fake_data_original[:, 0], bins=30, alpha=0.5, label='生成数据')
    plt.xlabel('数值')
    plt.ylabel('频数')
    plt.title('总排放量分布')
    plt.legend()
    ...
    plt.subplot(2, 2, 2)
    plt.hist(real_data_original[:, 1], bins=30, alpha=0.5, label='真实数据')
    plt.hist(fake_data_original[:, 1], bins=30, alpha=0.5, label='生成数据')
    plt.xlabel('数值')
    plt.ylabel('频数')
    plt.title('用电量分布')
    plt.legend()
    ...
    plt.subplot(2, 2, 3)
    plt.hist(real_data_original[:, 2], bins=30, alpha=0.5, label='真实数据')
    plt.hist(fake_data_original[:, 2], bins=30, alpha=0.5, label='生成数据')
    plt.xlabel('数值')
    plt.ylabel('频数')
    plt.title('废钢消耗量分布')
    plt.legend()
    ...
    plt.subplot(2, 2, 4)
    plt.hist(real_data_original[:, 3], bins=30, alpha=0.5, label='真实数据')
    plt.hist(fake_data_original[:, 3], bins=30, alpha=0.5, label='生成数据')
    plt.xlabel('数值')
    plt.ylabel('频数')
    plt.title('用烟煤量分布')
    plt.legend()
    ...
    # 保存生成的特征数据，使用pandas确保格式正确
    df_fake = pd.DataFrame(fake_data_original, columns=['总排放量', '用电量(MWh)', '废钢消耗量(t)', '一般烟煤消耗量(t)'])
    df_fake.to_csv('steel_data.csv', index=False)
    ...
    # 保存统计信息到单独文件
    stats_data = {
        '特征': ['总排放量(t)', '用电量(MWh)', '废钢消耗量(t)', '一般烟煤消耗量(t)'],
        '生成数据均值': mean_fake,
        '生成数据标准差': std_fake,
        '真实数据均值': mean_real,
        '真实数据标准差': std_real
    ...
    df_stats.to_csv('steel_data_stats.csv', index=False)
    print("数据已保存到 steel_data.csv")
    print("统计信息已保存到 steel_data_stats.csv")
...
if __name__ == "__main__":
    ...
    generator_meta, g_losses_meta, d_losses_meta = meta_train_gan(
        dataset, num_meta_epochs=12000, num_inner_epochs=3, 
