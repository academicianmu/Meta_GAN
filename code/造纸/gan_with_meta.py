...
class CementDataset(Dataset):
    def __init__(self, file_path):
        ...
        self.features = ["总排放量(tCO2)", "用电量(MWh)", "柴油消耗量(吨)"]
...
def plot_results(dataset, generator, g_losses, d_losses):
    ...
    # savedata四个生成特征数据,写上每一列的特征名
    np.savetxt('paper_data.csv', fake_data_original, delimiter=',', header='总排放量(tCO2),用电量(MWh),柴油消耗量(吨)')
    #计算mean和std与原始的对比，然后保存到刚才的data.csv文件的最后一行
    ...
    # 保存到data.csv文件的最后一行，有明显标识
    with open('paper_data.csv', 'a') as f:
        f.write('mean_fake,std_fake,mean_real,std_real\n')
        f.write(f'{mean_fake[0]},{std_fake[0]},{mean_real[0]},{std_real[0]}\n')
        f.write(f'{mean_fake[1]},{std_fake[1]},{mean_real[1]},{std_real[1]}\n')
        f.write(f'{mean_fake[2]},{std_fake[2]},{mean_real[2]},{std_real[2]}\n')
    print("数据已保存到 paper_data.csv")
...
if __name__ == "__main__":
    ...
    generator_meta, g_losses_meta, d_losses_meta = meta_train_gan(
        dataset, num_meta_epochs=20000, num_inner_epochs=3, 
